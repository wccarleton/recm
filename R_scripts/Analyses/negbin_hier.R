...
##RECTS
Y <- rects_sample[,2:dim(rects_sample)[2]]#sample(2:dim(rects_sample)[2],size=100,replace=FALSE)]
Y[which(is.na(Y))] <- 0
N <- dim(Y)[1]
K <- dim(Y)[2]
...
#save samples
save(samples,file="../Results/MCMC/Kennett/mcmc_samples_kennett_neg_hier_plotrun.RData")
