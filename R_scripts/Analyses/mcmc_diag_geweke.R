...
for(j in mcmc_results_files){
   ...
   write.csv(geweke_z,file=paste("../Results/MCMC/Geweke/geweke_",csv_file_name,".csv",sep=""))
}

#palaeoclimate function
mcmc_results_files <- list.files("../Results/MCMC/Kennett/")

for(j in mcmc_results_files){
   csv_file_name <- strsplit(j,split=".RData")[[1]]
   load(file=paste("../Results/MCMC/Kennett/",j,sep=""))
   niter <- dim(samples$samples)[1]
   mcmc_mat <- cbind(samples$samples,samples$samples2)
   geweke_z <- geweke.diag(mcmc_mat[burnin:niter,])$z
   print(j)
