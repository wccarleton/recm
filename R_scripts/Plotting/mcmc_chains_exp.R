...
samples <- read.csv("../Results/MCMC/Exp/mcmc_samples_exp_pos_hier.csv",as.is=T)
burnin <- 5000
niter <- dim(samples)[1]
...
samples <- read.csv("../Results/MCMC/Exp/mcmc_samples_exp_neg_hier.csv",as.is=T)
burnin <- 5000
niter <- dim(samples)[1]
noms <- colnames(samples)
cols <- grep("B",noms)[c(1,2)]
...
##Save
ggsave(filename=paste("../Images/mcmcchains_exp.png",sep=""),
      width=20,
      height=20,
      units="cm",
      scale=1,
